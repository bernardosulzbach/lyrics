...
class TestGuess(TestCase):
    def test_check_artist_base_case(self):
        song = lyrics.songs.Song("right", "title", ["A", "B"])
        right_guess = lyrics.guesses.Guess("right", "title")
        self.assertTrue(right_guess.check_artist(song))
        wrong_guess = lyrics.guesses.Guess("wrong", "title")
    ...
    def test_check_artist_should_ignore_whitespace(self):
        song = lyrics.songs.Song("right true", "title", ["A", "B"])
        right_guess = lyrics.guesses.Guess("right true", "title")
        self.assertTrue(right_guess.check_artist(song))
        right_guess = lyrics.guesses.Guess("right  true", "title")
        self.assertTrue(right_guess.check_artist(song))
        right_guess = lyrics.guesses.Guess("right   true", "title")
    ...
    def test_check_title_base_case(self):
        song = lyrics.songs.Song("artist", "right", ["A", "B"])
        right_guess = lyrics.guesses.Guess("artist", "right")
        self.assertTrue(right_guess.check_title(song))
        wrong_guess = lyrics.guesses.Guess("artist", "wrong")
    ...
    def test_check_title_should_ignore_whitespace(self):
        song = lyrics.songs.Song("artist", "right true", ["A", "B"])
        right_guess = lyrics.guesses.Guess("artist", "right true")
        self.assertTrue(right_guess.check_title(song))
        right_guess = lyrics.guesses.Guess("artist", "right  true")
        self.assertTrue(right_guess.check_title(song))
        right_guess = lyrics.guesses.Guess("artist", "right   true")
    ...
    def test_checks_should_allow_for_small_typos_in_artist(self):
        ...
        song = lyrics.songs.Song("Iron Maiden", "These Colours Don't Run", ["A", "B"])
        perfect_guess = lyrics.guesses.Guess("Iron Maiden", "These Colours Don't Run")
        self.assertTrue(perfect_guess.check_artist(song))
        self.assertTrue(perfect_guess.check_title(song))
        artist_one_character_typo = lyrics.guesses.Guess("Iron Maden", "These Colours Don't Run")
        ...
        artist_too_many_typos = lyrics.guesses.Guess("Iron Made", "These Colours Don't Run")
        self.assertFalse(artist_too_many_typos.check_artist(song))
        self.assertTrue(artist_too_many_typos.check_title(song))
        artist_even_more_typos = lyrics.guesses.Guess("Air Made", "These Colours Don't Run")
    ...
    def test_checks_should_allow_for_small_typos_in_title(self):
        ...
        self.assertTrue(perfect_guess.check_artist(song))
        self.assertTrue(perfect_guess.check_title(song))
        title_one_character_typo = lyrics.guesses.Guess("Iron Maiden", "These Colors Don't Run")
        self.assertTrue(title_one_character_typo.check_artist(song))
        self.assertTrue(title_one_character_typo.check_title(song))
        title_two_characters_typo = lyrics.guesses.Guess("Iron Maiden", "These Colors Dont Run")
        ...
        title_too_many_typos = lyrics.guesses.Guess("Iron Maiden", "This Colors Do Run")
        self.assertTrue(title_too_many_typos.check_artist(song))
        self.assertFalse(title_too_many_typos.check_title(song))
        title_even_more_typos = lyrics.guesses.Guess("Iron Maiden", "Deez Colors Do Run")
